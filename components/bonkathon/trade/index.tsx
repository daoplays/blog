...
import { useCallback, useEffect, useState, useRef, useMemo, SetStateAction, Dispatch } from "react";
...
import { PiArrowsOutLineVerticalLight, PiHouseBold, PiHouseLine } from "react-icons/pi";
...
export const checkBorrowCollection = async (
    ...
    let collection_account = PublicKey.findProgramAddressSync([amm_data_account.toBytes(), Buffer.from("Collection")], PROGRAM)[0];
...
const TradePage = ({ launch, setScreen }: { launch: AMMLaunch; setScreen?: Dispatch<SetStateAction<number>> }) => {
    ...
    const [user_quote_address, setUserQuoteAddress] = useState<PublicKey | null>(null);
    ...
    const [borrow_collection, setBorrowCollection] = useState<PublicKey | null>(null);

    // options state
    const [option_assets, setOptionAssets] = useState<AssetV1[]>([]);
    const [option_collection, setOptionCollection] = useState<PublicKey | null>(null);
    ...
    useEffect(() => {
        ...
        if (user_base_token_ws_id.current === null && user_base_address !== null) {
            user_base_token_ws_id.current = connection.onAccountChange(user_base_address, check_user_base_update, "confirmed");
        }
        if (user_quote_token_ws_id.current === null && user_quote_address !== null) {
            user_quote_token_ws_id.current = connection.onAccountChange(user_quote_address, check_user_quote_update, "confirmed");
        }
        ...
        if (collection_ws_id.current === null && borrow_collection !== null) {
            collection_ws_id.current = connection.onAccountChange(borrow_collection, check_collection_update, "confirmed");
        }

        if (options_ws_id.current === null && option_collection !== null) {
            options_ws_id.current = connection.onAccountChange(option_collection, check_options_update, "confirmed");
        }

        if (amm_ws_id.current === null && amm_address !== null) {
            amm_ws_id.current = connection.onAccountChange(amm_address, check_amm_update, "confirmed");
        }
    }, [
        connection,
        price_address,
    ...
    const CheckMarketData = useCallback(async () => {
        ...
        checkOptionsCollection(amm.base.mint, amm.quote.mint, setOptionAssets, setOptionCollection);
        ...
        let user_quote_amount = await request_token_amount(user_quote_token_account_key);
                    ...
                    <HStack spacing={3} align="start" justify="start">
                        ...
                        <Tooltip label="Copy Contract Address" hasArrow fontSize="large" offset={[0, 10]}>
                            <div
                                ...
                                onClick={(e) => {
                                    e.preventDefault();
                                    navigator.clipboard.writeText(amm.amm_data.base_mint.toString());
                                }}
                            >
    ...
    return (
        <>
            <HStack w="full" h="full" spacing={0} align="start" pb={sm ? 14 : 0}>
                {(!sm || (sm && (mobilePageContent === "Info" || mobilePageContent === "Trade"))) && (
                    ...
                    >
                        ...
                        <Box px={5} mt={-4} pb={5} width="100%" style={{ borderBottom: "1px solid rgba(134, 142, 150, 0.5)" }}>
                            <VStack>
                                <HStack>
                                    <WoodenButton action={() => setLeftPanel("Info")} label={"Info"} size={15} width="100%" />
                                    <WoodenButton
                                        action={() => {
                                            setLeftPanel("Trade");
                                        }}
                        ...
                        {(leftPanel === "Trade" || leftPanel === "LP" || leftPanel === "Borrow" || leftPanel === "Options") && (
                            <BuyAndSell
                ...
                {(!sm || (sm && mobilePageContent === "Chart")) && (
                    ...
                    >
                        ...
                        {selectedTab === "Options" && selectedOptionsTab === "Trade" && wallet.connected && (
                            <OptionsTable
                        ...
                        {selectedTab === "Options" && selectedOptionsTab === "Execute" && wallet.connected && (
                            <OptionsTable
                                base_2022={amm.base.token_program === TOKEN_2022_PROGRAM_ID}
                                base_mint={amm.base.mint}
                                quote_2022={amm.quote.token_program === TOKEN_2022_PROGRAM_ID}
                        ...
                        {selectedTab === "Options" && selectedOptionsTab === "Refund" && wallet.connected && (
                            <OptionsTable
                                base_2022={amm.base.token_program === TOKEN_2022_PROGRAM_ID}
                                base_mint={amm.base.mint}
                                quote_2022={amm.quote.token_program === TOKEN_2022_PROGRAM_ID}
                        ...
                        {selectedTab === "Shorts" && selectedShortsTab === "Exit" && wallet.connected && (
                            <ShortsTable
                        ...
                        {selectedTab === "Shorts" && selectedShortsTab === "Liquidate" && wallet.connected && (
                            <ShortsTable
                                is_2022={amm.base.token_program === TOKEN_2022_PROGRAM_ID}
                                amm={amm}
                                base_mint={amm.base.mint}
                        ...
                        {selectedTab === "Longs" && selectedShortsTab === "Exit" && wallet.connected && (
                            <ShortsTable
                                is_2022={amm.base.token_program === TOKEN_2022_PROGRAM_ID}
                                amm={amm}
                                base_mint={amm.base.mint}
                        ...
                        {selectedTab === "Longs" && selectedShortsTab === "Liquidate" && wallet.connected && (
                            <ShortsTable
                                is_2022={amm.base.token_program === TOKEN_2022_PROGRAM_ID}
                                amm={amm}
                                base_mint={amm.base.mint}
            ...
            {sm && (
                <Footer setScreen={setScreen} setLeftPanel={setLeftPanel} setMobilePageContent={setMobilePageContent} isTradePage={true} />
            )}
        </>
    );
};
...
const BuyAndSell = ({
    ...
    const { UpdateLiquidity, isLoading: updateLiquidityLoading } = useUpdateLiquidity();
    ...
    let base_raw = Math.floor(token_amount * Math.pow(10, base_data.mint.decimals));
    ...
    let quote_output = (base_input_amount * quote_balance) / (base_input_amount + base_balance) / Math.pow(10, quote_data.mint.decimals);

    let quote_raw = Math.floor(sol_amount * Math.pow(10, quote_data.mint.decimals));
    let amm_quote_fee = Math.ceil((quote_raw * amm.fee) / 100 / 100);
    let quote_input_amount = quote_raw - amm_quote_fee;

    let base_output = (quote_input_amount * base_balance) / (quote_balance + quote_input_amount) / Math.pow(10, base_data.mint.decimals);

    let price = quote_balance / Math.pow(10, quote_data.mint.decimals) / (base_balance / Math.pow(10, base_data.mint.decimals));

    let base_no_slip = sol_amount / price;
    let quote_no_slip = token_amount * price;

    let max_sol_amount = Math.floor(quote_no_slip * Math.pow(10, quote_data.mint.decimals) * 2);
    ...
    let lp_generated = (base_raw * (amm_lp_balance / base_balance)) / Math.pow(10, base_data.mint.decimals);

    let lp_quote_output = (quote_balance * base_raw) / amm_lp_balance / Math.pow(10, quote_data.mint.decimals);
    let lp_base_output = (base_balance * base_raw) / amm_lp_balance / Math.pow(10, base_data.mint.decimals);
    if (selected === "LP-") {
        quote_output_string = lp_quote_output <= 1e-3 ? lp_quote_output.toExponential(3) : lp_quote_output.toFixed(5);
        base_output_string = lp_base_output <= 1e-3 ? lp_base_output.toExponential(3) : lp_base_output.toFixed(base_data.mint.decimals);
    }

    let short_base_amount = short_amount * (1 - amm.fee / 100 / 100) * Math.pow(10, base_data.mint.decimals);
    let short_quote_output =
        (short_base_amount * quote_balance) / (short_base_amount + base_balance) / Math.pow(10, quote_data.mint.decimals);

    let liquidation_price = deposit_amount > 0 ? (deposit_amount + short_quote_output) / short_amount : short_quote_output / short_amount;
    let liquidation_price_string =
        liquidation_price <= 1e-3 ? liquidation_price.toExponential(3) : liquidation_price.toFixed(quote_data.mint.decimals);
        ...
        <VStack align="start" w="100%" h="100%" mt={-2} spacing={4}>
            {left_panel === "Options" ? (
                ...
                <VStack align="start" px={5} w="100%" mt={-2} spacing={4}>
                    ...
                    <HStack justify="space-between" w="100%" mt={left_panel === "Shorts" ? -2 : 2}>
                        <Text m={0} color={"white"} fontFamily="ReemKufiRegular" fontSize={"medium"} opacity={0.5}>
                            User Balance:
                        </Text>
                        <Text m={0} color={"white"} fontFamily="ReemKufiRegular" fontSize={"medium"}>
                            {selected === "Buy" || selected === "Short" || selected === "Long"
                                ? (bignum_to_num(user_quote_balance) / Math.pow(10, quote_data.mint.decimals)).toLocaleString("en-US", {
                                      minimumFractionDigits: 2,
                                  })
                                : selected === "LP-"
                                  ? (user_lp_balance / Math.pow(10, base_data.mint.decimals)).toLocaleString("en-US", {
                                        minimumFractionDigits: 2,
                                    })
                                  : (user_base_balance / Math.pow(10, base_data.mint.decimals)).toLocaleString("en-US", {
                                        minimumFractionDigits: 2,
                                    })}{" "}
                            {selected === "Buy" || selected === "Short" || selected === "Long"
                                ? quote_data.symbol
                    ...
                    {(selected === "Long" || selected === "Short") && (
                        <>
                            <HStack justify="space-between" w="100%" mt={2}>
                                ...
                                <Text m={0} color={"white"} fontFamily="ReemKufiRegular" fontSize={"medium"}>
                                    {selected === "Short" && bignum_to_num(amm.short_base_amount) / Math.pow(10, base_data.mint.decimals)}
                                    {selected === "Long" &&
    ...
    return (
        <VStack spacing={8} w="100%" mb={3}>
            <HStack mt={-2} px={5} justify="space-between" w="100%">
                ...
                <HStack>
                    <Text m={0} color={"white"} fontFamily="ReemKufiRegular" fontSize={"large"}>
                        {price < 1e-3 ? price.toExponential(3) : price.toFixed(Math.min(base_data.mint.decimals, 3))}
                    </Text>
            ...
            <HStack px={5} justify="space-between" w="100%">
                ...
                <Text m={0} color={"white"} fontFamily="ReemKufiRegular" fontSize={"large"}>
                    {(quote_amount / Math.pow(10, quote_data.mint.decimals)).toLocaleString("en-US", {
                        minimumFractionDigits: 2,
