...
export const OptionsPanel = ({
    ...
    const [localExpiryDate, setLocalExpiryDate] = useState<Date>(new Date(new Date().setHours(0, 0, 0, 0)));
    const [expiryDateAndTime, setExpiryDateAndTime] = useState("-- --");

    const { CreateOption, isLoading: isOptionLoading } = useCreateOption(symbol, uri, base_mint.mint.address.toString());
            ...
            <VStack align="start" w="100%">
                ...
                <InputGroup size="md">
                    ...
                    <InputRightElement h="100%" w={50}>
                        <img src={icon} width={30} height={30} alt="" style={{ borderRadius: "100%" }} />
                    </InputRightElement>
                </InputGroup>
                )
            </VStack>
            ...
            <VStack align="start" w="100%">
                ...
                <InputGroup size="md">
                    <Input
                        ...
                        onChange={(e) => {
                            setStrikePrice(
                                !isNaN(parseFloat(e.target.value)) || e.target.value === "" ? parseFloat(e.target.value) : strike_price,
                            );
                        }}
                        type="number"
                        min="0"
                    />
                    <InputRightElement h="100%" w={50}>
                        <Image src={quote_mint.icon} width={30} height={30} alt="SOL Icon" />
                    </InputRightElement>
                </InputGroup>
            </VStack>
            ...
            <VStack align="start" w="100%">
                ...
                <InputGroup size="md">
                    <Input
                        ...
                        onChange={(e) => {
                            setOptionPrice(
                                !isNaN(parseFloat(e.target.value)) || e.target.value === "" ? parseFloat(e.target.value) : option_price,
                            );
                        }}
                        type="number"
                        min="0"
                ...
                <div className={`${styles.textLabelInputDate} font-face-kg`}>
                    <HStack spacing={5}>
                        <Popover isOpen={isStartOpen} onClose={onCloseStart} placement="bottom" closeOnBlur={false}>
                            <PopoverTrigger>
                                <IconButton onClick={onToggleStart} aria-label="FaCalendarAlt" icon={<FaCalendarAlt size={22} />} />
                            </PopoverTrigger>
                        ...
                        <Text m="0" color="white" className="font-face-kg" fontSize={sm ? "small" : "small"}>
                            {expiryDateAndTime}
                        </Text>
                    </HStack>
                </div>
