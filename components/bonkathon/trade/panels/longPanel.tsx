...
function formatPrice(price: number, decimals: number) {
    let priceString = price <= 1e-3 ? price.toExponential(3) : price.toFixed(decimals);
...
const LongPanel = ({
    ...
    let quote_amount = (base_amount * quote_balance) / (base_balance - base_amount) / Math.pow(10, quote_data.mint.decimals);
    ...
    if (transfer_fee_config !== null) {
        total_base_fee += Number(calculateFee(transfer_fee_config.newerTransferFee, BigInt(raw_deposit_amount)));
    }
        ...
        <>
                ...
                <>
                    <InputGroup size="md">
                        <Input
                            ...
                            onChange={(e) => {
                                setShortAmount(
                                    !isNaN(parseFloat(e.target.value)) || e.target.value === "" ? parseFloat(e.target.value) : long_amount,
                                );
                            }}
                            type="number"
                            min="0"
            ...
            <>
                <Button
                    ...
                    onClick={() => {
                        !connected ? handleConnectWallet() : EnterShort(amm, long_amount, deposit_amount);
                    }}
                >
