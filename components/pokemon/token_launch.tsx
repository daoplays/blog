...
function GetCharityStats() {
  ...
  const init = useCallback(async () => {
    ...
    if (!have_charity_stats) {
      try {
        let program_data_key = await PublicKey.findProgramAddressSync(
        ...
        setTotalDonated(
          bignum_to_num(charity_data.donated_total) / LAMPORTS_PER_SOL,
        );

        let total_paid =
          bignum_to_num(charity_data.paid_total) / LAMPORTS_PER_SOL;
...
export function AirDropApp() {
  ...
  const join_ico = useCallback(async () => {
    ...
    console.log(
      "charity : ",
      charity_amount,
      charity_amount * LAMPORTS_PER_SOL,
      ch_bn.toNumber(),
    );
    console.log(
      "dao : ",
      dao_amount,
      dao_amount * LAMPORTS_PER_SOL,
      dao_bn.toNumber(),
    );
    ...
    const supporters_token_mint_key = new PublicKey(
      "7B1yoU3EsbABt1kNXcJLeJRT8jwPy9rZfhrhWzuCA9Fq",
    );
        ...
        <Box>
          ...
          <VStack alignItems="start" mt="2rem" mb="2rem">
            <Alert status="info">
              ...
              {average_price != null && (
                <Text>
                  To get double the tokens, and a DaoPlays Supporter Token, pay
                  more than the average price of {average_price.toFixed(4)} SOL!
                </Text>
              )}
            </Alert>
