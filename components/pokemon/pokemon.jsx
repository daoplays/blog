...
import { CategoryScale, LinearScale, BarElement, Title, Chart as ChartJS } from "chart.js";
...
const WalletDisconnectButtonDynamic = dynamic(async () => (await import("@solana/wallet-adapter-react-ui")).WalletDisconnectButton, {
    ssr: false,
});
...
const BidData = struct([u8("instruction"), u64("amount_charity"), u64("amount_daoplays"), u8("charity")]);
...
const u8_scheme = new Map([[my_u8, { kind: "struct", fields: [["value", "u8"]] }]]);

const u16_scheme = new Map([[my_u16, { kind: "struct", fields: [["value", "u16"]] }]]);

const u64_scheme = new Map([[my_u64, { kind: "struct", fields: [["value", "u64"]] }]]);

const pubkey_scheme = new Map([[my_pubkey, { kind: "struct", fields: [["value", [32]]] }]]);
        ...
        if (!have_charity_stats) {
            try {
                const auction_account_url =
                    `/.netlify/functions/solana_main?function_name=getAccountInfo&p1=` + auction_program_data_key.toString() + `&p2=base64`;
        ...
        try {
            if (!have_bidder_stats) {
                ...
                const program_account_url =
                    `/.netlify/functions/solana_main?function_name=getAccountInfo&p1=` + program_data_key.toString() + `&p2=base64`;
                ...
                let program_pda_key = (await PublicKey.findProgramAddress([Buffer.from("token_account")], program_key))[0];
                ...
                if (wallet.publicKey) {
                    ...
                    const bidder_account_url =
                        `/.netlify/functions/solana_main?function_name=getAccountInfo&p1=` + bidder_pda_key.toString() + `&p2=base64`;
                    ...
                    try {
                        ...
                        const bid_key = new PublicKey(
                            deserialize(pubkey_scheme, my_pubkey, program_account_data.slice(key_start_index, key_start_index + 32)).value,
                        );
                        ...
                        const bid_time = deserialize(
                            u64_scheme,
                            my_u64,
                            program_account_data.slice(time_start_index, time_start_index + 8),
                        ).value.toNumber();
        ...
        <Flex>
            {which_charity === "UkraineERF" && (
                <Card className="text-left" style={{ flexDirection: "row" }}>
                    <Card.Img style={{ width: "25%" }} src={UkraineERF_img.src} alt="banner" />
                    <Card.Body>
            ...
            {which_charity === "WaterOrg" && (
                <Card className="text-left" style={{ flexDirection: "row" }}>
                    <Card.Img style={{ width: "25%" }} src={WaterOrg_img.src} alt="banner" />
                    <Card.Body>
                        <Card.Text className="text-body mb-4" style={{ fontSize: "1rem" }}>
                            <br />
                            ...
                            Damon, Water.org pioneers market-driven financial solutions to the global water crisis. For 30 years, they've
                            been providing women hope, children health, and families a future. Find out more{" "}
            ...
            {which_charity === "OneTreePlanted" && (
                <Card className="text-left" style={{ flexDirection: "row" }}>
                    <Card.Img style={{ width: "25%" }} src={OneTreePlanted_img.src} alt="banner" />
                    <Card.Body>
                        <Card.Text className="text-body mb-4" style={{ fontSize: "1rem" }}>
                            <br />
            ...
            {which_charity === "EvidenceAction" && (
                <Card className="text-left" style={{ flexDirection: "row" }}>
                    <Card.Img style={{ width: "25%" }} src={EvidenceAction_img.src} alt="banner" />
                    <Card.Body>
                        <Card.Text className="text-body mb-4" style={{ fontSize: "1rem" }}>
                            <br />
                            Evidence Action is a global nonprofit organization with an approach distinctive in international development -
                            they exclusively scale interventions that are backed by strong evidence and can be delivered with exceptional
                            cost-effectiveness. Their programs have grown since their founding in 2013 to reach over 280 million people
                            annually. They take a data-driven approach to identifying, scaling, and continuously improving programs which
            ...
            {which_charity === "OutrightActionInt" && (
                <Card className="text-left" style={{ flexDirection: "row" }}>
                    <Card.Img style={{ width: "25%" }} src={Outright_img.src} alt="banner" />
                    <Card.Body>
                        <Card.Text className="text-body mb-4" style={{ fontSize: "1rem" }}>
                            <br />
            ...
            {which_charity === "TheLifeYouCanSave" && (
                <Card className="text-left" style={{ flexDirection: "row" }}>
                    <Card.Img style={{ width: "25%" }} src={LifeYouCanSave_img.src} alt="banner" />
                    <Card.Body>
                        <Card.Text className="text-body mb-4" style={{ fontSize: "1rem" }}>
                            <br />
                            The Life You Can Save is an advocacy nonprofit that makes “smart giving simple” by identifying and recommending
                            some of the world's most effective charities. They currently recommend over 20 outstanding charities whose
                            evidence-based, cost-effective interventions have been proven to save and transform the lives of people living
                            in extreme global poverty (defined as less than US$1.90 per day). They provide free tools and resources that
                            make it easy to learn about and support these wonderful organizations so that you can give where it matters most
                            and ensure that you get the most “bang for your buck.” Over the past three years, they've raised an average of
                            $17 for their recommended charities for each dollar they have spent on their own operations. Find out more{" "}
...
function GetTokens() {
    ...
    const join_ico = useCallback(async () => {
        ...
        let program_pda_key = (await PublicKey.findProgramAddress([Buffer.from("token_account")]["token_account"], program_key))[0];
        ...
        <>
            ...
            <Alert status="info" mb="1rem">
                ...
                <Text textAlign="center">
                    The greater your bid, the higher your chance of winning. New auctions take place every few seconds to minutes, and your
                    bid can remain active for multiple auctions. For more information check out our blog post{" "}
                    ...
                    <Box>
                        ...
                        <HStack>
                            ...
                            <FormControl id="charity_amount" maxWidth={"150px"}>
                                <Input type="text" value={(slide_value * sol_value * 0.01).toFixed(4)} readOnly />
                            </FormControl>
                        </HStack>
                        ...
                        <HStack>
                            ...
                            <FormControl id="dao_amount" maxWidth={"150px"} ml="2rem">
                                <Input type="text" value={((100 - slide_value) * sol_value * 0.01).toFixed(4)} readOnly />
                            </FormControl>
                        </HStack>
                    </Box>
                    ...
                    <VStack borderWidth="2px" borderRadius="2rem" p="1rem" spacing="2rem">
                            ...
                            <HStack>
                                ...
                                <VStack>
                                    <FormControl id="n_bidders" maxWidth={"120px"}>
                                        <Input type="text" value={n_bidders != null ? n_bidders : "Loading.."} readOnly />
                                    </FormControl>
                                    <FormControl id="total_bid" maxWidth={"120px"}>
                                        <Input
                                            type="text"
                                            value={
                                                total_bid != null ? (total_bid === 0 ? 0 : (total_bid / n_bidders).toFixed(4)) : "Loading.."
                                            }
                                            readOnly
                                        />
                                    </FormControl>
                                    <FormControl id="tokens_available" maxWidth={"120px"}>
                                        <Input type="text" value={tokens_remaining ? tokens_remaining : "Loading.."} readOnly />
                                    </FormControl>
                                </VStack>
                            </HStack>
                        </VStack>
                            ...
                            <HStack>
                                ...
                                <VStack>
                                    <FormControl id="current_bid" maxWidth={"120px"}>
                                        <Input type="text" value={current_bid != null ? current_bid.toFixed(4) : "Loading.."} readOnly />
                                    </FormControl>
                                    ...
                                    <FormControl id="bid_time" maxWidth={"120px"}>
                                        <Input type="text" value={current_bid && bid_index != null ? bid_index : 0} readOnly />
                                    </FormControl>
                                </VStack>
                            </HStack>
                        </VStack>
                        ...
                        <Alert status="info" mb="1rem">
                            <AlertIcon />
                            {
                                <Text>
                                    {" "}
                                    The program will send tokens and select new winners whenever anyone bids or plays the game. If there are
                                    only a few people playing right now you can click the button below to do this yourself.
                                </Text>
                            }
                        </Alert>
    ...
    const press_button_wrapper = useCallback(
        async ({ button_type, button_bid }) => {
            ...
            if (team_name !== "") {
                ...
                const button_url =
                    `/.netlify/functions/post_db?team_name=` + team_name + "&button=" + button_type + "&bid=" + button_bid_value;
        ...
        <>
                ...
                <VStack>
                    ...
                    <HStack>
                        ...
                        <FormControl id="team_name" maxWidth={"150px"}>
                            <Input type="text" value={team_name} onChange={handleTeamNameChange} />
                        </FormControl>
                    </HStack>
                </VStack>
            </Center>
            ...
            <Center>
                <HStack spacing="25px" mt="1rem">
                    <VStack spacing="10px">
                        <IconButton onClick={press_up_button} aria-label="press up" icon={<FaCaretSquareUp size="80" />} />
                        <HStack spacing="55px">
                            <IconButton onClick={press_left_button} aria-label="press left" icon={<FaCaretSquareLeft size="80" />} />
                            <IconButton onClick={press_right_button} aria-label="press right" icon={<FaCaretSquareRight size="80" />} />
                        </HStack>
                        <IconButton onClick={press_down_button} aria-label="press down" icon={<FaCaretSquareDown size="80" />} />
                    </VStack>
                        ...
                        <Box fontSize="17">
                            <VStack>
                                <FormControl id="balance" maxWidth={"175px"}>
                                    <Input type="text" value={lamports ? lamports / LAMPORTS_PER_SOL : "Loading.."} readOnly />
                                </FormControl>
                                <FormControl id="tokenbalance" maxWidth={"175px"}>
                                    <Input type="text" value={token_amount ? token_amount : "0"} readOnly />
                                </FormControl>
                                <FormControl id="supporterbalance" maxWidth={"175px"}>
                                    <Input type="text" value={supporter_amount ? supporter_amount : "0"} readOnly />
                                </FormControl>
                            </VStack>
                        </Box>
                    </HStack>
    ...
    return (
        <>
            ...
            <Alert status="info" mb="1rem">
                ...
                {
                    <Text textAlign="center">
                        Take part in the DaoPlays Pokemon token launch to pay what you want for an initial block of Play Tokens{" "}
            ...
            <Center>
                <Flex flexDirection="row">
                    ...
                    <Box ml="1rem" borderWidth="2px" borderRadius="2rem" p="1rem">
                        <TwitchChat channel="daoplays_" id="daoplays_" theme="dark" height="480" />
                        <Divider mt="1rem" />
                        <AccountInfo />
                    </Box>
                </Flex>
