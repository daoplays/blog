...
const HOOK_PROGRAM = new PublicKey(
  "vyyNeAorB3Ce4nyfBK4dL7CMu9Jx2M9vK8zBGvFrpYd",
);
...
class Transfer_Token_Instruction {
  ...
  static readonly struct = new BeetStruct<Transfer_Token_Instruction>(
    ...
    (args) => new Transfer_Token_Instruction(args.instruction!, args.quantity!),
    "Transfer_Token_Instruction",
  );
}

class MintCounter {
  constructor(readonly counter: bignum) {}
...
function Tokens2022App() {
  ...
  const [processing_transaction, setProcessingTransaction] =
  ...
  const CheckTempKeypair = useCallback(async () => {
    ...
    if (check_hook_transfers.current === true) {
      let mint_data_account = await PublicKey.findProgramAddressSync(
        [Buffer.from("mint_data"), current_mint.toBytes()],
        HOOK_PROGRAM,
      );
      try {
        ...
        const [counter] = MintCounter.struct.deserialize(
  ...
  useEffect(() => {
    if (check_keypair_interval.current === null) {
      check_keypair_interval.current = window.setInterval(
        CheckTempKeypair,
        1000,
      );
    } else {
    ...
    for (const accountInfo of allAccounts) {
      ...
      if (
        ...
        console.log(
          ...
          (
            parseInt(transferFeeAmount.withheldAmount.toString()) / 1000
          ).toString(),
        );
  ...
  const GetTokenAmounts = useCallback(async () => {
    ...
    let current_balance = await connection.getTokenAccountBalance(
      user_token_key,
      "processed",
    );
    ...
    const transaction = new Transaction().add(
      createAmountToUiAmountInstruction(
        current_mint,
        parseInt(current_balance.value.amount),
        TOKEN_2022_PROGRAM_ID,
      ),
    );
    const { returnData, err } = (
      await connection.simulateTransaction(transaction, [temp_keypair], false)
    ).value;
    ...
    if (current_balance.value.uiAmount !== null)
      setTokenBalance(current_balance.value.uiAmount);
  ...
  const Transfer = useCallback(async () => {
    ...
    let init_address = PublicKey.findProgramAddressSync(
      [Buffer.from("extra-account-metas"), current_mint.toBytes()],
      HOOK_PROGRAM,
    )[0];
    let mint_data_address = PublicKey.findProgramAddressSync(
      [Buffer.from("mint_data"), current_mint.toBytes()],
      HOOK_PROGRAM,
    )[0];
    ...
    if (include_default) {
      let thaw_idx = createThawAccountInstruction(
        new_token_key,
        current_mint,
        wallet.publicKey,
        undefined,
    ...
    try {
      ...
      var transaction_response = await send_transaction(
        "",
        encoded_transaction,
      );
  ...
  const Create = useCallback(async () => {
    ...
    let init_address = PublicKey.findProgramAddressSync(
      [Buffer.from("extra-account-metas"), mint_pubkey.toBytes()],
      HOOK_PROGRAM,
    )[0];
    let mint_data_address = PublicKey.findProgramAddressSync(
      [Buffer.from("mint_data"), mint_pubkey.toBytes()],
      HOOK_PROGRAM,
    )[0];
            ...
            <>
              <VStack>
                {transfer_accounts.map((item: TransferAccount, index) => (
                  <Text key={index} mb="0">
                    {item.pubkey.toString().slice(0, 5) + "..."} withheld{" "}
                    {item.amount}
                  </Text>
                ))}
              </VStack>
      ...
      <Center mb="5rem" width="100%">
        <VStack>
          ...
          {processing_transaction && (
            <Image src={loading.src} width="50px" alt={""} />
          )}
        </VStack>
      </Center>
    );
