...
const TradePage = ({ launch }: { launch: AMMLaunch }) => {
    ...
    const CheckMarketData = useCallback(async () => {
        ...
        console.log("user amounts", user_base_amount, user_lp_amount, base_amount, quote_amount);
...
const ShortCard = ({ asset, amm, base_mint, quote_mint }: { asset: AssetV1; amm: AMMData; base_mint: MintData; quote_mint: MintData }) => {
    const wallet = useWallet();
    ...
    let current_price = quote_post_fees / Math.pow(10, quote_mint.mint.decimals) / (base_input / Math.pow(10, base_mint.mint.decimals));

    let current_time = new Date().getTime() / 1000;
    let time_delta_years = (current_time - start_time) / 60 / 60 / 24 / 365;
    let borrow_fee = Math.floor(time_delta_years * quote_input * (amm.borrow_cost / 100) + 1) / Math.pow(10, quote_mint.mint.decimals);
...
const BuyAndSell = ({
    ...
    if (transfer_fee_config !== null) {
        ...
        max_transfer_fee = Number(transfer_fee_config.newerTransferFee.maximumFee) / Math.pow(10, base_data.mint.decimals);
        ...
        <VStack align="start" px={5} w="100%" mt={-2} spacing={4}>
            ...
            <HStack justify="space-between" w="100%" mt={2}>
                ...
                <Text m={0} color={"white"} fontFamily="ReemKufiRegular" fontSize={"medium"}>
                    {selected === "Buy"
                        ? (user_quote_balance / Math.pow(10, quote_data.mint.decimals)).toLocaleString("en-US", {
                    ...
                    {selected === "Buy" ? quote_data.symbol : selected === "LP-" ? "LP" : base_data.symbol}
                </Text>
            </HStack>
            <HStack justify="space-between" w="100%" mt={2}>
                <Text m={0} color={"white"} fontFamily="ReemKufiRegular" fontSize={"medium"} opacity={0.5}>
            ...
            {selected === "Exit" && (
                <>
                    {owned_assets.map((asset, i) => (
                        <ShortCard key={i} asset={asset} amm={amm} base_mint={base_data} quote_mint={quote_data} />
                    ))}
                </>
            )}
            ...
            {selected === "LP+" ? (
                <Button
                    ...
                    onClick={() => {
                        !wallet.connected ? handleConnectWallet() : UpdateLiquidity(amm, token_amount, 0);
                    }}
                >
                    <Text m={"0 auto"} fontSize="large" fontWeight="semibold">
                ...
                <Button
                    ...
                    isLoading={updateLiquidityLoading}
                    onClick={() => {
                        !wallet.connected ? handleConnectWallet() : UpdateLiquidity(amm, token_amount, 1);
                    }}
                >
                    <Text m={"0 auto"} fontSize="large" fontWeight="semibold">
                ...
                <>
                    <Button
                        ...
                        onClick={() => {
                            !wallet.connected ? handleConnectWallet() : EnterShort(amm, short_amount, deposit_amount);
                        }}
                    >
                        <Text m={"0 auto"} fontSize="large" fontWeight="semibold">
                ...
                <>
                    <Button
                        ...
                        isLoading={placingOrder}
                        onClick={() => {
                            !wallet.connected ? handleConnectWallet() : PlaceMarketOrder(amm, token_amount, sol_amount, order_type);
                        }}
                    >
                        <Text m={"0 auto"} fontSize="large" fontWeight="semibold">
