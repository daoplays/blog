...
export class Swap_Instruction {
    ...
    static readonly struct = new FixableBeetStruct<Swap_Instruction>(
        ...
        (args) => new Swap_Instruction(args.instruction!, args.side!, args.in_amount!),
        "PlaceLimit_Instruction",
    );
}

export function serialise_swap_instruction(side: number, in_amount: number): Buffer {
...
const usePlaceMarketOrder = () => {
    ...
    const PlaceMarketOrder = async (amm_data : AMMData, token_amount: number, sol_amount: number, order_type: number) => {
        const connection = new Connection(DEV_RPC_NODE, { wsEndpoint: DEV_WSS_NODE });
        ...
        let quote_amount = new BN(sol_amount * Math.pow(10, quote_mint_data.decimals));
        ...
        const instruction_data = serialise_swap_instruction(order_type, order_type === 0 ? quote_amount : base_amount);
