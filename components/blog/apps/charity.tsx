...
export function GetCharityStats() {
    ...
    useEffect(() => {
        if (check_charity_stats_interval.current === null) {
            check_charity_stats_interval.current = window.setInterval(get_charity_stats, 1000);
        } else {
...
export function useSolanaAccount() {
    ...
    const check_sol = useCallback(async () => {
        if (wallet.publicKey) {
            ...
            try {
                let aWalletMyTokenBalance = await connection.getTokenAccountBalance(this_token_pubkey);
            ...
            try {
                let aWalletMyTokenBalance = await connection.getTokenAccountBalance(this_supporter_pubkey);
                let token_amount = parseInt(aWalletMyTokenBalance["value"].amount);
    ...
    }, [wallet, connection, token_pubkey, supporter_pubkey, lamports_amount, supporter_raw_amount, token_raw_amount]);
...
export const DrawChartMemo = memo(
    function DrawChartNew({ x_labels, y_data, bar_colours } : {x_labels : string[], y_data : number[], bar_colours : string[]}) {
        ...
        return(
            <Bar redraw data={data2} options={options}/>
    ...
    return (
        <Box textAlign="center" fontSize="l">
            ...
            <Flex flexDirection="row">
                ...
                <Box flex="1" pl="2rem">
                    <DrawChartMemo 
                    x_labels= {["UkraineERF", "Water.Org", "OneTreePlanted", "EvidenceAction", "GirlsWhoCode", "OutrightAction", "TheLifeYouCanSave"]} 
