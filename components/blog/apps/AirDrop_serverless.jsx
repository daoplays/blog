...
function useSolanaAccount() {
  ...
  const init = useCallback(async () => {
    if (!have_account_info && wallet.publicKey) {
      const url =
        `/.netlify/functions/solana_dev?function_name=getAccountInfo&p1=` +
...
export function AirDropApp() {
  ...
  const getAirdrop = useCallback(async () => {
    ...
    try {
      const url =
        `/.netlify/functions/solana_dev?function_name=requestAirdrop&p1=` +
        wallet.publicKey.toString() +
        `&p2=` +
      ...
      const signature_url =
        `/.netlify/functions/sig_status_dev?function_name=getSignatureStatuses&p1=` +
        airdropSignature;
      const signature_result = await fetch(signature_url).then((res) =>
        res.json(),
      );
      const confirmation_status =
        signature_result["result"]["value"][0]["confirmationStatus"];

      if (confirmation_status === "finalized") have_account_info = false;
    } catch (error) {
      console.log(error);
    }
  ...
  const handle_transaction = useCallback(async () => {
    ...
    try {
      ...
      const send_url =
        `/.netlify/functions/solana_dev?function_name=sendTransaction&p1=` +
        encoded_transaction;
      const transaction_result = await fetch(send_url).then((res) =>
        res.json(),
      );
