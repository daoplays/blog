...
const team_lookup_schema = new Map([
  [TeamLookupData, { kind: "struct", fields: [["team_account", [32]]] }],
]);
...
const myu64_schema = new Map([
  [MyU64, { kind: "struct", fields: [["number", "u64"]] }],
]);
...
function useDataAccount() {
  ...
  const init = useCallback(async () => {
    ...
    let data_key = (
      await PublicKey.findProgramAddress(["data_account"], program_key)
    )[0];
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (team_num["number"].toNumber() > 0) {
        ...
        let team_lookup_key = (
          await PublicKey.findProgramAddress([team_buffer], program_key)
        )[0];
        //console.log("key ", team_lookup_key.toString());
        let team_lookup_account =
          await connection.getAccountInfo(team_lookup_key);
        const team_lookup_data = deserialize(
          team_lookup_schema,
          TeamLookupData,
          team_lookup_account.data,
        );
        ...
        const team_data = deserialize(
          team_data_schema,
          TeamData,
          team_data_account.data,
        );
        ...
        let team_name = new TextDecoder().decode(
          team_data["team_name"].slice(0, name_len),
        );
...
function useCurrentTeamAccount() {
  ...
  const init = useCallback(async () => {
    ...
    if (current_team !== "") {
      let team_data_key = (
        await PublicKey.findProgramAddress([current_team], program_key)
      )[0];
      let team_data_account = await connection.getAccountInfo(team_data_key);
      const team_data = deserialize(
        team_data_schema,
...
function usePicture() {
  ...
  const init = useCallback(async () => {
    if (!have_picture) {
      ...
      setPicture(
        "https://images.unsplash.com/photo-1497034825429-c343d7c6a68f?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=687&q=80",
      );
...
export function IceCreamApp() {
  ...
  const Create = useCallback(async () => {
    ...
    let team_data_key = (
      await PublicKey.findProgramAddress([desired_team_name], program_key)
    )[0];
    ...
    const create_team_meta = new TeamMeta({
      instruction: IceCreamInstruction.create_team,
      team_name: desired_team_name.toString(),
    });
    ...
    let sysvar_rent_key = new PublicKey(
    ...
    const create_team_instruction = new TransactionInstruction({
      keys: [
        ...
        {
          pubkey: team_token_mint_pubkey,
          isSigner: token_is_signer,
          isWritable: token_is_signer,
        },
        {
    ...
    let team_lookup_key = (
      await PublicKey.findProgramAddress([team_lookup_index_data], program_key)
    )[0];

    //console.log("team lookup key: ", team_lookup_key.toString());

    const create_team_lookup_meta = new TeamMeta({
      instruction: IceCreamInstruction.create_team_lookup,
      team_name: desired_team_name.toString(),
    });
    const create_team_lookup_data = serialize(
      team_meta_schema,
      create_team_lookup_meta,
    );
  ...
  const Login = useCallback(async () => {
    ...
    if (check_team_name !== "") {
      ...
      let team_data_key = (
        await PublicKey.findProgramAddress([check_team_name], program_key)
      )[0];
      ...
      let saved_team_name = new TextDecoder().decode(
        team_data["team_name"].slice(0, name_len),
      );
      ...
      let token_balance = await connection.getTokenAccountBalance(token_key);
  ...
  const Eat = useCallback(async () => {
    ...
    let team_data_key = (
      await PublicKey.findProgramAddress([team_name], program_key)
    )[0];
    ...
    const eat_meta = new TeamMeta({
      instruction: IceCreamInstruction.eat,
        ...
        <TabPanels>
          <TabPanel>
            ...
            {wallet.publicKey && (
              <>
                <Text mt="2rem" mb="1rem" textAlign="center" fontSize="1.5rem">
                  Step 1: Log in or create a new team
                </Text>
                        ...
                        <HStack marginBottom="10px">
                          <VStack alignItems="start">
                            <Box height="40px">
                              <Tooltip
                                hasArrow
                                label="Desired Team Name.  Must be unique amongst all existing teams."
                              >
                                <Text mt="7px">Team Score</Text>
                              </Tooltip>
                            </Box>
                            <Box height="40px">
                              <Tooltip
                          ...
                          <VStack>
                            ...
                            <Box height="40px" width="450px">
                              <a
                                ...
                                href={
                                  "https://solscan.io/account/" + team_token
                                }
                              >
                                <FormControl
                                  id="check_team_name"
                        ...
                        <HStack marginBottom="10px">
                          <VStack>
                            <Box height="40px">
                              <Tooltip
                                hasArrow
                                label="Valid Team Name.  You will need to own the correct token for the team."
                              >
                                <Text mt="7px">Team Name</Text>
                              </Tooltip>
                            </Box>
                            <Box height="40px"></Box>
                          </VStack>
                    ...
                    <HStack marginBottom="10px">
                      <VStack>
                        ...
                        <Box height="40px">
                          <Tooltip
                            hasArrow
                            label="(Optional) Pass an existing token mint address to create a team using that token.  It must have zero decimal places, and you must be the mint authority.)"
                          >
                            <Text mt="7px">Token Mint</Text>
                          </Tooltip>
                        </Box>
                      </VStack>
                ...
                {team_name !== "" && (
                  <>
                    <Text
                      ...
                      fontSize="1.5rem"
                    >
                      Step 2: Eat (Click) the food to score points for your
                      team!
                    </Text>
                    <Center>
                        ...
                        <Figure>
                          ...
                          <Figure.Caption>
                            ...
                            <a
                              style={{ textDecoration: "underline" }}
                              href={
                                "https://unsplash.com/@sadswim?utm_source=ice_cream_battle&utm_medium=referral"
                              }
                            >
                              {artist}
                            </a>{" "}
                            ...
                            <a
                              style={{ textDecoration: "underline" }}
                              href={
                                "https://unsplash.com/?utm_source=ice_cream_battle&utm_medium=referral"
                              }
                            >
                              Unsplash
                            </a>
