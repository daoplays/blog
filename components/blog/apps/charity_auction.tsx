...
export class BidderData {
  ...
  static readonly struct = new BeetStruct<BidderData>(
    [["index", u16]],
    (args) => new BidderData(args.index!),
    "BidderData",
  );
}
...
function GetBidderStats() {
  ...
  const init = useCallback(async () => {
    const program_key = new PublicKey(
      "EzigyiBDJy7Srq8xn6SK6Nx7BpenbSE3YbBSaBpPSN1q",
    );
    ...
    try {
      if (wallet.publicKey) {
        ...
        let program_data_account =
          await connection.getAccountInfo(program_data_key);
        let program_pda_key = (
        ...
        let bidder_data_account =
          await connection.getAccountInfo(bidder_pda_key);
        ...
        let aWalletMyTokenBalance =
          await connection.getTokenAccountBalance(program_token_key);

        if (program_data_account === null) return;

        const [auction_data] = AuctionData.struct.deserialize(
          program_data_account.data,
        );

        setTokensRemaining(parseInt(aWalletMyTokenBalance.value.amount));
        setTotalBid(
          bignum_to_num(auction_data.total_bid_amount) / web3.LAMPORTS_PER_SOL,
        );
        ...
        if (bidder_data_account !== null) {
          const [bidder_data] = BidderData.struct.deserialize(
            bidder_data_account.data,
          );

          const bid_key = auction_data.bid_keys[bidder_data.index];
          const bid_amount = bignum_to_num(
            auction_data.bid_amounts[bidder_data.index],
          );
          const bid_time = bignum_to_num(
            auction_data.bid_times[bidder_data.index],
          );
...
function GetCharityStats() {
  ...
  const get_charity_stats = useCallback(async () => {
    ...
    const auction_program_key = new PublicKey(
      "EzigyiBDJy7Srq8xn6SK6Nx7BpenbSE3YbBSaBpPSN1q",
    );
    const daoplays_key = new web3.PublicKey(
      "2BLkynLAWGwW58SLDAnhwsoiAuVtzqyfHKA3W3MJFwEF",
    ...
    try {
      let auction_data_key = await PublicKey.createWithSeed(
        daoplays_key,
        "data_account",
        auction_program_key,
      );
      let auction_data_account =
        await connection.getAccountInfo(auction_data_key);

      let launch_program_data_key = await PublicKey.findProgramAddressSync(
        [Buffer.from("token_account")],
        launch_program_key,
      );
      let launch_program_data_account = await connection.getAccountInfo(
        launch_program_data_key[0],
      );

      if (auction_data_account === null || launch_program_data_account === null)
        return;

      const [launch_data] = CharityData.struct.deserialize(
        launch_program_data_account.data,
      );
      const [auction_data] = CharityData.struct.deserialize(
        auction_data_account.data.slice(49299, 49379),
      );

      let total_donated =
        (bignum_to_num(launch_data.donated_total) +
          bignum_to_num(auction_data.donated_total)) /
        web3.LAMPORTS_PER_SOL;

      setTotalDonated(total_donated);

      let n_donations =
        bignum_to_num(launch_data.n_donations) +
        bignum_to_num(auction_data.n_donations);
      ...
      let new_donation_array = [
        (bignum_to_num(launch_data.charity_totals[0]) +
          bignum_to_num(auction_data.charity_totals[0])) /
          web3.LAMPORTS_PER_SOL,
        (bignum_to_num(launch_data.charity_totals[1]) +
          bignum_to_num(auction_data.charity_totals[1])) /
          web3.LAMPORTS_PER_SOL,
        (bignum_to_num(launch_data.charity_totals[2]) +
          bignum_to_num(auction_data.charity_totals[2])) /
          web3.LAMPORTS_PER_SOL,
        (bignum_to_num(launch_data.charity_totals[3]) +
          bignum_to_num(auction_data.charity_totals[3])) /
          web3.LAMPORTS_PER_SOL,
        (bignum_to_num(launch_data.charity_totals[4]) +
          bignum_to_num(auction_data.charity_totals[4])) /
          web3.LAMPORTS_PER_SOL,
        (bignum_to_num(launch_data.charity_totals[5]) +
          bignum_to_num(auction_data.charity_totals[5])) /
          web3.LAMPORTS_PER_SOL,
        (bignum_to_num(launch_data.charity_totals[6]) +
          bignum_to_num(auction_data.charity_totals[6])) /
          web3.LAMPORTS_PER_SOL,
      ];
...
export function AirDropApp() {
  ...
  const join_ico = useCallback(async () => {
    ...
    const pyth_btc = new web3.PublicKey(
      "HovQMDrbAgAYPCmHVSrezcSmkMtXSSUsLDFANExrZh2J",
    );
    const pyth_eth = new web3.PublicKey(
      "EdVCmQ9FSPcVe5YySXDPCRmc8aDQLKJ9xvYBMZPie1Vw",
    );
    const pyth_sol = new web3.PublicKey(
    ...
    const bid_instruction_data = new Place_Bid_Instruction(
      AuctionInstruction.place_bid,
      ch_bn,
      dao_bn,
      chosen_charity,
    );
    const [bid_buffer] =
      Place_Bid_Instruction.struct.serialize(bid_instruction_data);
    ...
    const select_data = new Basic_Instruction(
      AuctionInstruction.select_winners,
    );
        ...
        <Box>
                ...
                <HStack>
                  <VStack width="100%" alignItems="start">
                    <HStack width="100%">
                      ...
                      <Text>
                        {" "}
                        {n_bidders != null ? n_bidders : "Loading.."}{" "}
                      </Text>
                    </HStack>
                    ...
                    <HStack width="100%">
                      ...
                      <Text>
                        {" "}
                        {tokens_remaining ? tokens_remaining : "Loading.."}{" "}
                      </Text>
                    </HStack>
                  </VStack>
                </HStack>
                ...
                <HStack>
                  <VStack alignItems="start">
                    ...
                    <HStack>
                      ...
                      <Text>
                        {" "}
                        {current_bid && bid_index != null ? bid_index : 0}{" "}
                      </Text>
                    </HStack>
                  </VStack>
                </HStack>
            ...
            {tokens_remaining != null && tokens_remaining < 100 ? (
              <>
                <Alert status="error" mb="1rem">
                  ...
                  {
                    <Text>
                      Less than one hundred tokens remaining. Cannot select
                      winners until more are available
                    </Text>
                  }
                </Alert>
              ...
              <>
                <Alert status="error" mb="1rem">
                  ...
                  {
                    <Text>
                      No bids found in the system. Cannot select winners until a
                      bid has been made
                    </Text>
                  }
                </Alert>
              ...
              <>
                <Alert status="success" mb="1rem">
                  ...
                  {
                    <Text>
                      You have been selected! Click the button to get your
                      tokens
                    </Text>
                  }
                </Alert>
                ...
                300.0 / n_bidders +
                  2 -
                  (Math.floor(Date.now() / 1000) - time_selected),
                0,
              ) > 0 ? (
              <>
              ...
              <>
                <Alert status="info" mb="1rem">
                  ...
                  {
                    <Text>
                      The program will send tokens and select new winners
                      whenever anyone bids or plays the game. If there aren't
                      many people playing right now you can click the button
                      below to do this yourself.
                    </Text>
                  }
                </Alert>
                <Button
          ...
          <VStack alignItems="start" mt="2rem" mb="2rem">
            <Alert status="info" mb="1rem">
              ...
              {total_bid != null && (
                <Text>
                  Your chance of winning is proportional to the amount you bid.
                  If you bid the current average of
                  {total_bid === 0 ? 0 : (total_bid / n_bidders).toFixed(4)} SOL
                  you will have a one in
                  {current_bid ? n_bidders : n_bidders + 1} chance of winning in
                  the next draw
                </Text>
              )}
            </Alert>
