...
function SetCollectionOptions({
  ...
  const { CreateCollection, isLoading: isCollectionLoading } =
    useCreateCollection(collection_name, collection_uri, setCollection);
...
function SetTokenOptions({
  ...
  const { CreateNFT, isLoading: isNFTLoading } = useCreateNFT(
    token_name,
    token_uri,
    collection,
    setToken,
  );
  const { TransferNFT, isLoading: isTransferLoading } = useTransferNFT(
    collection,
    token,
    destination,
    setToken,
      ...
      <VStack w="100%" style={{ paddingBottom: md ? 35 : "75px" }}>
        <HStack>
          ...
          <Link
            ...
            href={
              "https://core.metaplex.com/explorer/collection/" +
              collection.toString() +
              "?env=devnet"
            }
            target="_blank"
              ...
              <>
                ...
                <HStack mt={md ? 0 : 30}>
                  <button
                    type="button"
                    disabled={
                      isNFTLoading || isTransferLoading || token === null
                    }
                    onClick={(e) => {
                      TransferNFT();
                    }}
...
function App() {
  ...
  return (
    <>
      ...
      {wallet.publicKey && collection === null && (
        <SetCollectionOptions setCollection={setCollection} />
      )}
      {wallet.publicKey && collection !== null && (
        <SetTokenOptions
          token={token}
