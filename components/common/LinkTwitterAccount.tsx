...
const TwitterIntegration = () => {
    ...
    if (check_twitter_user.current) {
      try {
        ...
        const snapshot = await get(
          ref(database, "BlinkBash/twitter_" + wallet.publicKey.toString())
        );
  ...
  const fetchUserInfo = async () => {
    try {
      const response = await fetch(
        "/.netlify/functions/fetchTwitterUser?user_key=" +
  ...
  const initiateTwitterLogin = async () => {
    try {
      const response = await fetch(
        "/.netlify/functions/twitterAuth?user_key=" +
          wallet.publicKey.toString(),
        { method: "GET" }
      );
  ...
  const handleTweetSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "/.netlify/functions/postTweet?user_key=" + wallet.publicKey.toString(),
        {
          method: "POST",
