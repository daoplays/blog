...
function PostContent() {
    ...
    return (
        ...
        <div className="container">
            ...
            One of our main goals in developing DaoPlays Pokemon was to make charitable giving a key component of the app.  For example, in the token launch, users can pay what they want for a block of Play Tokens, and can decide how much stays with us at DaoPlays and how much we donate to charity, with all donations handled via <a  style={{textDecoration: "underline"}}  href="https://thegivingblock.com/">The Giving Block</a>. 

            <br/><br/>

            In this post we won't be going through the technical aspects of how the app works, however we will reference relevant blog posts that do go through the code in detail.  Here we will be focussing on how to interact with the app, and a high level overview of what it does in the following sections:

            <br/><br/>
            <ul>
                <li>
                    <HStack><Text align="center" m="0" p="0">Getting tokens from the token launch (time limited) </Text><a href="#token_launch-header"><AiOutlineArrowRight /></a></HStack>
                </li>
                <li>
                    <HStack><Text align="center" m="0" p="0">Getting tokens from the token auction </Text><a href="#token_auction-header"><AiOutlineArrowRight/></a></HStack>
                </li>
                <li>
                    <HStack><Text align="center" m="0" p="0">Spending tokens to vote for moves </Text><a  href="#play_game-header"><AiOutlineArrowRight/></a></HStack>
                </li>
            </ul>

            <br/>
            
            If you are brand new to Solana we can recommend giving our 'Getting Started With Solana' blog post a read <Link style={{textDecoration: "underline"}} href="/blog/solana_getting_started">here</Link>.  Once you are set up with a wallet such as <a  style={{textDecoration: "underline"}}  href="https://phantom.app/">Phantom</a>, you can go and get your SOL either from a centralized exchange like <a  style={{textDecoration: "underline"}}  href="https://www.coinbase.com/home">Coinbase</a>, or if you already own other crypto assets from one of the many different decentralized exchanges out there.
            ...
            You can read a detailed description of the token launch program <Link style={{textDecoration: "underline"}} href="/blog/charity_token_launch">here</Link>.  This program will only be running for a limited time, but until it closes it is the easiest way to get an initial block of 1000-2000 Play Tokens.  You can find the token launch page <Link style={{textDecoration: "underline"}} href="/pokemon/token_launch">here</Link>.
            ...
            In order for the app to actually take the move, we are monitoring the Solana blockchain using the approach similar to the one we described in <Link style={{textDecoration: "underline"}} href="/blog/solana_streamer">this</Link> post. Every time a new block is confirmed a move is either taken based on the transactions included in that block, or if there were no transactions then no move is taken and time simply advances in the game.  Please note that although the Solana blockchain is very fast, and it takes very little time for your transaction to be confirmed, there is latency associated with Twitch, and so you can expect to wait about ten seconds for the move selected from a given block to be shown in the game stream.  You may also see that at times the game speeds up and slows down.  This occurs whenever the <a style={{textDecoration: "underline"}} href="https://www.quicknode.com/">QuickNode</a> endpoint that we are using to monitor the transactions on the block chain lags behind reality slightly.  In order to ensure that the game is able to catch up again once it has found out what moves it should make, it will increase the game speed, so that although all the moves still take up the same number of game frames, it will take less real time to play them.  One of our future goals is to invest in our own endpoint that we can run locally which will hopefully resolve those issues in the future.
