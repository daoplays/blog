...
function PostContent() {
    ...
    return (
        ...
        <div className="container">
            ...
            The main loop for our monitoring system is found in <Code p="0"><a style={{textDecoration: "underline"}} href="https://github.com/daoplays/solana_examples/blob/master/solana_streamer/python/streamer.py">streamer.py</a></Code>,  which starts by establishing a connection to the SQL database:
            ...
            Here we are making use of the python <Code p="0"><a style={{textDecoration: "underline"}} href="https://docs.python.org/3/library/sqlite3.html">sqlite3</a></Code> module to manage our database, which we have called <Code p="0">solana_block_data.db</Code>, and use the <Code p="0"><a style={{textDecoration: "underline"}} href="https://docs.python.org/3/library/sqlite3.html#sqlite3.connect">connect</a></Code> function to open the connection.  Note that we are setting the <Code p="0"><a style={{textDecoration: "underline"}} href="https://docs.python.org/3/library/sqlite3.html#sqlite3.Connection.isolation_level">isolation_level</a></Code> to <Code p="0">None</Code> here, which means that we will be explicitly controlling the start and end of the transactions that will be adding rows into our database, rather than have the python module handle this in the background for us.  If for some reason this fails the function will return <Code p="0">None</Code> and the main code in <Code p="0">streamer.py</Code> will immediately exit with an error.  If it succeeds it will call the <Code p="0">create_table</Code> function shown below:

            <HighLightCode codeBlock={create_table} language={"python"}/>


            The inclusion of the <Code p="0">IF_NOT_EXISTS</Code> statement in <Code p="0">create_table_idx</Code> ensures that this instruction will only actually do anything if the table doesn't already exist in the database.  In this case it will create a table with four columns, an <Code p="0">id</Code> which is simply the row index and provides a unique identifier for each row, <Code p="0">block_slot</Code> which is the slot number for a particular block, and finally <Code p="0">choice</Code> and <Code p="0">bid_amount</Code> which are the quantities that users can pass to our program that we want to keep track of, and will determine the evolution of our off-chain application.  If for some reason this process fails it will return <Code p="0">False</Code> and the main program will exit immediately with an error.
            ...
            This function, shown below, creates an SQL query that will return the row that has the maximum value of <Code p="0">id</Code> in the database.  If there are no entries it will return <Code p="0">None</Code>, and otherwise it will return the row.  Note this is a very slow function to call, and this is the only time that we make use of it.  Typically we will be tracking the current row id using the <Code p="0">current_row_id_to_insert</Code> variable in <Code p="0">streamer.py</Code>, and incrementing it as we  iterate through the main loop.
            ...
            If the database doesn't currently exist, and <Code p="0">get_last_db_row</Code> returned <Code p="0">None</Code>, then we will set the current row id to zero, and use the <Code p="0">get_slot</Code> function to retrieve the current slot number being worked on.   Just a quick note here about blocks and slots; every block that is confirmed on the Solana blockchain has a corresponding slot, and in these cases the slot number and block number tend to be used interchangeably.  Not all slots, however, have a block (we will come back to this later), and when using <Code p="0">get_slot</Code> you can pass a <Code p="0">commitment</Code> argument to specify how certain you want to be that the slot returned will actually have a block associated with it.  By default this commitment level is set to <Code p="0">finalized</Code>, which means the slot will definitely have a block.

            <HighLightCode codeBlock={streamer_state_3} language={"python"}/>


            The <Code p="0"><a style={{textDecoration: "underline"}} href="https://docs.solana.com/developing/clients/jsonrpc-api#getslot">get_slot</a></Code> function is the first example in this post of a call to the Solana JSON RPC <a style={{textDecoration: "underline"}} href="https://docs.solana.com/developing/clients/jsonrpc-api">API</a>. Two of the three requests follow this same format (the other being <Code p="0">get_blocks</Code> which we will use in the next section).  As the request to the API is asynchronous, there is no guarantee that it will return successfully, as it may time out or some other problem may occur while processing the request.  We therefore set up a <Code p="0">While</Code> loop that tries to get a response, and catches any errors in order to simply try again after a short wait (0.25 seconds in our example).           

            <HighLightCode codeBlock={get_slot} language={"python"}/>


            The responses from these requests should include either a <Code p="0">result</Code> node, or an <Code p="0">error</Code> node indicating something has gone wrong with the request.  Sometimes however, a problem may occur when sending or receiving the request which causes neither of these to be present.  We therefore define a simple helper function <Code p="0">check_json</Code> to return   <Code p="0">True</Code> if the result node is present, and otherwise return <Code p="0">False</Code>, logging the error if it was present:
            ...
            <ul>
                <li>
                    <HStack><Text align="center" m="0" p="0">request the set of finalized block numbers since the last iteration</Text> <a href="#block-number-header"><AiOutlineArrowRight/></a></HStack>
                    </li>
                <li>
                    <HStack><Text align="center" m="0" p="0">request the data for those blocks </Text><a href="#get-blocks-header"><AiOutlineArrowRight/></a></HStack>
                </li>
                <li>
                    <HStack><Text align="center" m="0" p="0">process that data and store it in our database </Text><a href="#get-data-header"><AiOutlineArrowRight/></a></HStack>
                </li>
            </ul>
            <br/>
            ...
            Unlike other blockchains like Ethereum, some blocks on the Solana blockchain can be '<a style={{textDecoration: "underline"}} href="https://support.quicknode.com/hc/en-us/articles/5793700679441-Why-are-slots-blocks-missing-on-Solana-">skipped</a>', meaning they will contain no data. When trying to get the set of blocks that we should request, we just want to be able to ignore these skipped blocks, and the RPC API provides the function <Code p="0"><a style={{textDecoration: "underline"}} href=" https://docs.solana.com/developing/clients/jsonrpc-api#getblocks">get_blocks</a></Code> to do precisely that.  We just pass as an argument the most recent valid block number that we know about, and it will return the list of finalized blocks from that number, up to the most recent.
            
            
            <br/><br/>
            As with <Code p="0">get_slot</Code>, we wrap the <Code p="0">get_blocks</Code> function in a <Code p="0">While</Code> loop within our <Code p="0">get_block_list</Code> function, shown below.   We also use our <Code p="0">check_json</Code> function again here to verify whether the response is as expected, and in this case we also check that the list of blocks is not empty.  


            <HighLightCode codeBlock={get_blocks} language={"python"}/>


            Within the main loop of our monitor, we check if the last entry of the list returned by <Code p="0">get_block_list</Code> is the same as <Code p="0">current_block</Code>.  If it is then we know that no new blocks have been finalized, and so we simply wait a short time and then check again.  Once we have new blocks within the list we simply remove the first entry and proceed to the next step.
            ...
            Unlike the previous two RPC requests, in this case we want to be able to make multiple requests to the <Code p="0"><a style={{textDecoration: "underline"}} href=" https://docs.solana.com/developing/clients/jsonrpc-api#getblock">get_block</a></Code> RPC function.  As such  we will create a batch request and use the python <Code p="0">requests</Code> module to post that to our endpoint.  We handle creating the batch requests in the <Code p="0">make_blocks_batch_request</Code> function in <Code p="0"><a style={{textDecoration: "underline"}} href="https://github.com/daoplays/solana_examples/blob/master/solana_streamer/python/rpc_funcs.py">rpc_funcs.py</a></Code>.  

            <HighLightCode codeBlock={make_blocks_batch_request_0} language={"python"}/>



            Here <Code p="0">dev_client_url</Code> is the URL of our RPC endpoint and <Code p="0">block_list</Code> is the list of slots that we recieved from <Code p="0">get_slot</Code> previously.  In order to track the success of each request within the batch we use <Code p="0">have_block</Code>, which is vector of bools that has the same length as <Code p="0">block_list</Code>, and finally <Code p="0">blocks</Code> is a map that will contain the responses for each block within the batch.
            ...
            The header contains only a single entry for the node <Code p="0">Content-Type</Code>, which must be set to <Code p="0">application/json</Code>.  We define a template request using <Code p="0">new_request</Code>, which we just assign a default id and slot of zero.  Most of the settings in the params node are straight forward, though the <Code p="0">maxSupportedTransactionVersion</Code> setting is a relatively new addition and is required to support blocks that have transactions using the <a style={{textDecoration: "underline"}} href="https://docs.rs/solana-program/latest/solana_program/message/index.html">v0</a> message type, as opposed to only "legacy", which is still the default.

            <br/><br/>

            For each block in the list we use this template to create a new request to <Code p="0">getBlock</Code>, set the slot appropriately, and increment the id number, before adding it to the <Code p="0">request_vec</Code> vector.

           
            <HighLightCode codeBlock={make_blocks_batch_request_2} language={"python"}/>        


            With the batch request constructed we can then post it to our endpoint using the <Code p="0">requests</Code> module.  As with the standard RPC API requests, we still need to manage the times where the request times out, or some other error occurs.  As before we therefore wrap the post method in a <Code p="0">while</Code> loop that retries until a response has been received.  If the request has returned successfully it will have a <Code p="0">status_code</Code> of 200, so we check this and simply return out of the function if it is not the case (we will handle this in the next function), and otherwise convert the response into a json object to make it easier to parse.
            
            <HighLightCode codeBlock={make_blocks_batch_request_3} language={"python"}/>   

            This response object will be a vector of length the number of blocks requested, where each entry is the response for the specific block.  Any one of these could in principle have failed for some reason, and so we iterate through the list, checking if there is a result node.  If there is we can insert this into our <Code p="0">blocks</Code> map using the slot number as the key, and mark the entry in the <Code p="0">have_block</Code> vector as <Code p="0">True</Code> so that we know this block has been received.

            <br/><br/>

            This function now returns the map, and the vector which denotes which of the blocks has been successfully requested.  We handle the case where only a subset of the desired blocks have been received in the <Code p="0">get_one_block_batch</Code> function which we show below.

            
            <HighLightCode codeBlock={get_one_block_batch} language={"python"}/>


            This function is simply responsible for repeatedly calling <Code p="0">make_blocks_batch_request</Code> until the whole of the <Code p="0">have_block</Code> vector of bools has been set to <Code p="0">True</Code>.  For each iteration in the loop it will pass the current state both of this vector, and the blocks map back to <Code p="0">make_blocks_batch_request</Code>, which will only send requests for the blocks that are still missing.  Once it has finished requesting all the blocks in the batch it then returns the map.
            ...
            We therefore have one final layer where we take the initial <Code p="0">block_list</Code> that is returned in the main loop, and break it up into chunks of one hundred blocks, and then process each of these chunks separately.  This is done in the <Code p="0">get_blocks</Code> function, which we will now describe below.
            

            <HighLightCode codeBlock={get_blocks_from_list} language={"python"}/>


            The first thing we need to do in this function is determine the number of batches we will have to process given our <Code p="0">batch_size</Code> of one hundred.  If this is only one, as will typically be the case, we can just call <Code p="0">get_one_block_batch</Code> directly and no more needs to be done.

            <HighLightCode codeBlock={get_blocks_from_list_2} language={"python"}/>

            If we have more than one batch then we create a vector that will contain the slot numbers that each batch will be responsible for.  For each entry in <Code p="0">batch_lists</Code> we can then call <Code p="0">get_one_block_batch</Code>, and then build up a single <Code p="0">blocks</Code> map from the results of each batch.  These requests are managed by pythons <Code p="0">ThreadPoolExecutor</Code>, which can submit multiple batches in parallel and then process the results as they complete.  At least with the end point we are using we have found it to be more reliable and faster to maintain a running pool of ten parallel requests, each for 100 blocks, than to submit batches of one thousand blocks in serial, however this may well depend on the end point so if you are implementing something like this yourself, you may just want to test out a range of batch and pool sizes.
            ...
            Everything that we are interested in is found in the <Code p="0">message</Code> node that exists within each transaction.   This contains an <Code p="0">instructions</Code> node which lists the instructions executed in that transaction, and an <Code p="0">accountKeys</Code> node which lists the public keys used during the transaction, including the programs that were called. Within each instruction there is a <Code p="0">programIdIndex</Code>, which gives the index into <Code p="0">accountKeys</Code> that provides the program public key that executed the instruction.  
            
            <br/><br/>

            In order to find which instructions are relevant to our program then, we simply iterate through all the instructions within each transaction and compare the program public key given by <Code p="0">programIdIndex</Code> with the public key of our program. 

            <HighLightCode codeBlock={process_block_2} language={"python"}/>

            Once we have found an instruction of interest, we can access the data from the instruction's <Code p="0">data</Code> node, which is encoded as a base58 string.  We can then use the base58 python module to decode this into a byte array, and then use the <Code p="0"><a style={{textDecoration: "underline"}} href="https://near.github.io/borsh-construct-py/">borsh_construct</a></Code> module to convert that into a human readable data structure, however this final step requires knowledge of the data structures that the program will be passed.

            <br/><br/>

            In the <a style={{textDecoration: "underline"}}  href="https://github.com/daoplays/solana_examples/tree/master/solana_streamer/program">program</a> source code for this example we can see the definition of the instructions that our on-chain program will accept.  In this case there is only one, and it takes a <Code p="0">ChoiceData</Code> structure as an argument.

            <HighLightCode codeBlock={program_state} language={"python"}/>

            The <Code p="0">ChoiceData</Code> structure simply contains a <Code p="0">Choice</Code> enum, and an unsigned 64bit integer representing a quantity.  
            ...
            These objects allow us to try and convert a byte array into the given structure type using the <Code p="0">parse</Code> function:

            <HighLightCode codeBlock={process_block_3} language={"python"}/>

            If this is successful we can then also check that the instruction is of the type we are interested in using the <Code p="0">isinstance</Code> function.  In this example this is trivially true because the program only accepts a single instruction, however in the more general case you may want to distinguish between different program instructions, and only save data for a subset.  If we are interested in the data then we append it to the <Code p="0">data_vec</Code>, which we then return when we are done parsing all the instructions in the block.

            <br/><br/>

            At this point we are almost done! Given a vector of instruction data we convert that into the correct format for our database with the <Code p="0">create_rows_from_data</Code> function:

            <HighLightCode codeBlock={create_rows_from_data} language={"python"}/>

            Here we denote an empty block as a row with <Code p="0">no_choice</Code> and amount 0, and otherwise access the <Code p="0">choice</Code> and <Code p="0">bid_amount</Code> fields from our <Code p="0">ChoiceData</Code> structure to create each row, incrementing the <Code p="0">row_id</Code> as we go.  These rows can then be added to our database with the <Code p="0">insert_rows</Code> function in sql_funcs.py:
            ...
            As with requesting the block data, if we are just processing a single block within a particular iteration of the main loop then these functions are just called directly.  Otherwise if there are multiple blocks to be processed we once again use the <Code p="0">concurrent.futures</Code> python module to multithread the processing and create all the new rows in parallel.  Once all the blocks within an iteration have been processed the complete set of new rows are then added to the database in one go in the correct order, as shown below:

            <HighLightCode codeBlock={streamer_4} language={"python"}/>


            The final step in the loop is to simply update <Code p="0">current_block</Code> with the last block from <Code p="0">block_list</Code>, and then the loop repeats again.



            <h3 id="test-header" className="mt-5" style={{fontSize: "20px"}}>Testing The Monitoring Server</h3><br />

            The monitoring server can be started simply by running <Code p="0">python streamer.py</Code> in the <Code p="0">python</Code> directory of this post's source code.  This will start looking for interactions with a very simple program we are running on the Solana devnet, the code for which is located in the <Code p="0">program</Code> directory, and for which there is a rust client in the <Code p="0">client</Code> directory.
