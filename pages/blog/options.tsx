  ...
  return (
    <div className="home" style={{ fontSize: 18 }}>
      <div className="container">
        ...
        You'll find the app below, just enter a token-2022 mint address that exists on devnet (for example, Di4T8BWnEmB4erdBifoj8xTGaRSEcDEZei9CuXzurEwn, which you can buy from Lets Cook on devnet https://devnet.letscook.wtf/trade/blueman) and you'll be able to create, trade and execute options with that token as an underlying asset.  
        <br />
        <br />
        <OptionsApp />
        ...
        Ultimately core assets are just accounts on chain that can contain arbitrary data, where the standard makes it easy to interact with those accounts, transferring ownership between users, or updating the data on chain.  When you create an option is uses the core standard to store all of the details for the option in the attributes plugin for the asset.  This means that the program can access and update that metadata on chain.  Below you'll see the metadata we store for each option.  This includs things like the side (Call or Put), the strike price, the creator and so on.  When you relist an option that you bought previously the program will just update the fields for the option price and the seller, and this will be displayed in any explorer that supports core assets.
