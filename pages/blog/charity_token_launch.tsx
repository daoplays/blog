...
function PostContent() {
    ...
    return (
        ...
        <div className="container">
            ...
            <ul>
                <li>
                    <HStack><Text align="center" m="0" p="0">Create two different Solana tokens and give them names and icons </Text><a href="#tokens-header"><AiOutlineArrowRight/></a></HStack>
                </li>
                <li>
                    <HStack><Text align="center" m="0" p="0">Use TGB to create wallets for our chosen charities</Text> <a href="#charity-header"><AiOutlineArrowRight/></a></HStack>
                </li>
                <li>
                    <HStack><Text align="center" m="0" p="0">Create the token launch Dapp that incorporates giving through TGB, and rewards users who pay more than the average </Text><a href="#program-header"><AiOutlineArrowRight/></a></HStack>
                </li>
                <li>
                    <HStack><Text align="center" m="0" p="0">Test out the launch Dapp with a simple front-end interface</Text> <a href="#app-header"><AiOutlineArrowRight/></a></HStack>
                </li>
            ...
            <br/>
            <ul>
                <li><Code p="0">init_token_launch</Code>: Initialises the token launch, creates the program's data account and transfers tokens over to the program</li>
                <li><Code p="0">join_token_launch</Code>: Allows users to participate in the token launch</li>
                <li><Code p="0">end_token_launch</Code>: Ends the token launch, and transfers any remaining tokens back from the program</li>
            ...
            <ul>
                <li>
                    <HStack><Text align="center" m="0" p="0">Creating the program's data account</Text> <a href="#pda-header"><AiOutlineArrowRight/></a></HStack>
                </li>
                <li>
                    <HStack><Text align="center" m="0" p="0">Creating the program's token accounts for each of our tokens</Text> <a href="#ata-header"><AiOutlineArrowRight/></a></HStack>
                </li>
                <li>
                    <HStack><Text align="center" m="0" p="0">Transferring our tokens from our personal wallet to the program to manage  during the token launch</Text> <a href="#transfer-header"><AiOutlineArrowRight/></a></HStack>
                </li>
            </ul>
            ...
            If you look at the <a style={{textDecoration: "underline"}} href="https://docs.rs/solana-program/latest/src/solana_program/pubkey.rs.html#456-459">source</a> for this function, you can see that all it is doing is calling the <Code p="0">create_program_address</Code> function using  both the provided seed, and a separate bump seed which starts at a value of 255  and decreases by one until the function finds a valid PDA. Once it has been found it is recommended to directly call <Code p="0">create_program_address</Code> on chain and simply pass the correct bump_seed, rather than reusing <Code p="0">find_program_address</Code> as the former can be significantly less costly.

            <br/><br/>

            Now that we know where the program's account is going to live we need to actually create it, and for this we make use of Solana's <Code p="0"><a style={{textDecoration: "underline"}} href="https://docs.rs/solana-program/1.4.18/solana_program/system_instruction/fn.create_account.html">create_account</a></Code> function:

            <br /><br />
            <HighLightCode codeBlock={create_account} language={"rust"}/>
            <br /> 

            The <Code p="0">from_pubkey</Code> refers to the wallet address that will fund the creation of the account, and the <Code p="0">to_pubkey</Code> is simply the address where we want to create our new account which will be the PDA.  The third pubkey that is passed as the last argument, <Code p="0">owner</Code>, is the address of our program.   In english then, this will create a new account for which our program will be the owner, at the location of our newly found PDA, and we will fund it's creation with our own wallet.
            ...
            Once we have this address we can then simply call <Code p="0"><a style={{textDecoration: "underline"}} href="https://docs.rs/spl-associated-token-account/latest/spl_associated_token_account/instruction/fn.create_associated_token_account.html">create_associated_token_address</a></Code> to actually create the account:
            ...
            To do this we will be making use of the spl_token function <Code p="0"><a  style={{textDecoration: "underline"}}  href="https://docs.rs/spl-token/latest/spl_token/instruction/fn.transfer.html">transfer</a></Code>:

            <br /><br />
            <HighLightCode codeBlock={transfer_function} language={"rust"}/>
            <br />

            The arguments here are relatively self explanatory, the <Code p="0">authority_pubkey</Code> in this case will be our wallet, however when the program is transferring tokens to other users that will be the PDA.  In our use cases we will be leaving the signers vector empty.  Our complete function to handle transferring the tokens is shown below, and it's structure should by now by quite familiar!  We simply create the instruction, and then call <Code p="0">invoke_signed</Code> to handle the cross program invocation and actually enact the transaction.  Note that in this case we could have used <Code p="0">invoke</Code>, as our PDA doesn't need to sign this transaction, however when the program is sending out tokens then we do need to use <Code p="0">invoke_signed</Code> and it is simpler to just use it generically for both cases.
           ...
           <br/>
            ...
            Here <Code p="0">amount_charity</Code> and <Code p="0">amount_dao</Code> are the amounts in lamports to be paid to the chosen charity, and to the developers respectively.  The <Code p="0">Charity</Code> type of the final member of this struct refers to an enum we have defined in <Code p="0">state.rs</Code>:
            ...
            We next check if the joiners associated token account already exists by using <Code p="0"><a style={{textDecoration: "underline"}}  href="https://docs.rs/solana-program/latest/solana_program/account_info/struct.AccountInfo.html#method.try_borrow_lamports">try_borrow_lamports</a></Code>, which will return a positive value if it has been previously initialised and there are lamports present in it. If it doesn't exist we create it using our <Code p="0">create_token_account</Code> function:
            ...
            Next we want to check whether the participant has paid more than the current average, in which case they are awarded supporter status, and so will receive double the number of DPTTs, and an additional supporters token.  The current average price is stored in the program's data account, so we first have to retrieve that using the <Code p="0"><a style={{textDecoration: "underline"}} href="https://docs.rs/borsh/latest/borsh/de/trait.BorshDeserialize.html#method.try_from_slice">try_from_slice</a></Code> function from the <Code p="0"><a style={{textDecoration: "underline"}} href="https://docs.rs/crate/borsh/latest">borsh</a></Code> crate.  As a last check before proceeding we simply make sure there are enough DPTTs remaining.  We don't need to do this for the supporters tokens as we have made sure there is a sufficient supply even if every participant is a supporter.
            ...
            In order to transfer SOL from one account to another on chain we once again make use of the <Code p="0">invoke</Code> function, this time executing the <Code p="0"><a style={{textDecoration: "underline"}} href="https://docs.rs/solana-sdk/latest/solana_sdk/system_instruction/fn.transfer.html">transfer</a></Code> instruction, first to send the charity's amount, and then the developer's:
            ...
            The last step in this function is to update all the launch statistics with the new participants payment and donation.  We once again use the <Code p="0">borsh</Code> library to send the current state to the program's data account using the <Code p="0"><a style={{textDecoration: "underline"}} href="https://docs.rs/borsh/latest/borsh/ser/trait.BorshSerialize.html#tymethod.serialize">serialize</a></Code> function:
            ...
            The only new function we will make use of here is the spl_token instruction <Code p="0"><a style={{textDecoration: "underline"}}    href="https://docs.rs/spl-token/latest/spl_token/instruction/fn.close_account.html">close_account</a></Code>:
            ...
            This can only be called on a token account that has no tokens remaining, and will take care of transferring the lamports to the <Code p="0">destination_pubkey</Code> account.  The reason that we can't simply do this ourselves with a transfer instruction is that neither we nor the program actually own the token account, the token program does, and so the token program must be the one that sends us the lamports.

            <br/><br/>

            We define our <Code p="0">close_program_token_account</Code> function below, which takes the our program account and it's token account, our wallet and the destination token account and the token program itself:
            ...
            As in our <Code p="0">join_token_launch</Code> function we make use of the <Code p="0">unpack_unchecked</Code> spl_token function to grab the token account's data so that we can retrieve the remaining token balance.  If there are any tokens left we then make use of our <Code p="0">transfer_token</Code> function to send them to the <Code p="0">destination_token_account</Code>:
            ...
            As the last step we now make use of the spl_token <Code p="0">close_account</Code> function, and call <Code p="0">invoke_signed</Code> to allow our program to sign the transaction:
            ...
            Our <Code p="0">end_token_launch</Code> function then simply has to call this function for both the main token account, and the supporters token account after parsing and checking the account details as usual.
