...
`
    return (
        ...
        <div className="container">
            ...
            Over the last few years Deep Reinforcement Learning (RL) has made remarkable progress in being applied to ever more complex environments, from <Link style={{textDecoration: "underline"}}  href="https://www.nature.com/articles/nature16961">Go</Link>, <Link  style={{textDecoration: "underline"}} href="https://cdn.openai.com/dota-2.pdf">DOTA2</Link> and <Link style={{textDecoration: "underline"}} href="https://www.deepmind.com/blog/alphastar-mastering-the-real-time-strategy-game-starcraft-ii">StarCraft II</Link>, up to more recent developments in open ended games like <Link style={{textDecoration: "underline"}}  href="https://minerl.io/">Minecraft</Link>. Broadly speaking the  RL learning loop involves an agent operating in some environment, making observations and taking actions.  Some actions may result in a reward (positive or negative), for example winning a game, and the agent learns a policy for taking moves in order to maximise its long term reward.
            
            <br/><br/>

            For many interesting applications, however, encoding how the environment should hand out rewards and penalities (the reward function) is extremely challenging, and attempts to hard code this has yielded <Link style={{textDecoration: "underline"}}  href="https://towardsdatascience.com/how-learning-reward-functions-can-go-wrong-6e794e42f4fc">many</Link> examples of misaligment between what the agent ends up doing, and what the human designer intended. 

            <br/><br/>

            One potential avenue that attempts to solve this problem is Deep Reinforcement Learning From Human Feedback (RLHF).  In their <Link style={{textDecoration: "underline"}}   href = "https://arxiv.org/pdf/1706.03741.pdf">paper</Link> (henceforth C17) the authors augment the RL process by having humans watch pairs of videos that show the agent behaving in different ways, and provide their preferences for which video is closer to displaying the behaviour desired from the problem.  From these preferences a reward function is trained using a supervised learning process, and that trained reward function can then used as part of the RL process instead of relying on a hand coded function.  In the paper the authors use this approach to train an agent to play atari games, and simulate robot locomotion, such as making a <i>Hopper</i> robot do backflips:
            
            <br/><br/>
            <Center>
                <VStack>
                    <Image src={hopper.src}/>
                    <Text>Image from <Link  style={{textDecoration: "underline"}} href="https://openai.com/research/learning-from-human-preferences">OpenAI</Link> showing their hopper robot doing backflips</Text>
                </VStack>
            </Center>
            <br/>
            ...
            Since then RLHF has been used to great effect to produce fine tuned large language models such as <Link  style={{textDecoration: "underline"}}  href="https://arxiv.org/abs/2203.02155">InstructGPT</Link>, and has been a significant factor in making ChatGPT so successfull.  You can find <Link  style={{textDecoration: "underline"}} href="https://huggingface.co/blog/rlhf">articles</Link> out there that go through the details of how to implement RLHF in this kind of context.

            <br/><br/>

            In this post we will take a bottom up approach, introducing some of the core concepts from RLHF, and applying them to a much simpler problem, the <Link style={{textDecoration: "underline"}}  href="https://github.com/jakal02/gym-moving-dot">moving-dot</Link> gym environment.  This problem simply involves a dot that is spawned at a random location in a 2D space, and must move towards the center of the screen. A single episode lasts for 2000 timesteps, during which time the environment provides a score of +1 if the agent moves closer to the center, and -1 if the agent moves further away. 
            <br/><br/>

            <Center>
                <VStack>
                    <Image src={moving_dot.src} alt="moving dot"/>
                    <Text>Example of an optimised moving-dot, originally from <Link  style={{textDecoration: "underline"}} href="https://github.com/jakal02/gym-moving-dot">here</Link>.</Text>
                </VStack>
            </Center>
            ...
            <ul>
                <li>
                    <HStack><Text align="center" m="0" p="0">Some technical background on the RLHF implementation</Text> <a href="#background"><AiOutlineArrowRight/></a></HStack>
                </li>
                <li>
                    <HStack><Text align="center" m="0" p="0">Generating the preferences database </Text><a href="#preferences"><AiOutlineArrowRight/></a></HStack>
                </li>
                <li>
                    <HStack><Text align="center" m="0" p="0">Training a reward model using those preferences</Text><a  href="#reward"><AiOutlineArrowRight/></a></HStack>
                </li>
                <li>
                    <HStack><Text align="center" m="0" p="0">Training our moving dot agent given the reward model </Text><a href="#training"><AiOutlineArrowRight/></a></HStack>
                </li>
            </ul>
            ...
            <br/><br/>
            <MathJaxContext>
            First, if we denote the <i>i</i>th observation of a particular environment as <MathJax inline>{"\\(o_i\\)"}</MathJax>, and the action taken as a result of making that observation as  <MathJax inline>{"\\(a_i\\)"}</MathJax>, we can write a sequence of <i>k</i> observation-action pairs, a <i>trajectory segment</i>, as <MathJax inline>{"\\(\\sigma = (o_0, a_0),(o_1,a_1),\\dots,(o_{k-1}, a_{k-1})\\)"}</MathJax>. 
            
            <br/><br/>
            In C17 humans were asked to compare two such segments, which we will refer to as <MathJax inline>{"\\(\\sigma_1\\)"}</MathJax> and <MathJax inline>{"\\(\\sigma_2\\)"}</MathJax>, and to simply record either which segment they prefer, that they have no preference, or that they are unable to compare the two segments. These opinions are then stored in a database and used to train a reward model using supervised learning.  In particular, if we write the total reward for trajectory segment <MathJax inline>{"\\(\\sigma_1\\)"}</MathJax> as {" " }
            
            <MathJax inline>{"\\(R_1 = r(o_0, a_0) + r(o_1,a_1)+\\dots+r(o_{k-1}, a_{k-1})\\)"}</MathJax>, then if the user prefers sequence <MathJax inline>{"\\(\\sigma_1\\)"}</MathJax> to sequence <MathJax inline>{"\\(\\sigma_2\\)"}</MathJax>, written as <MathJax inline>{"\\( \\sigma_1 \\succ\\sigma_2\\)"}</MathJax>, then we want to have <MathJax inline>{"\\( R_1 > R_2\\)"}</MathJax>.
            </MathJaxContext>
            ...
            From this the <Link style={{textDecoration: "underline"}}  href="https://en.wikipedia.org/wiki/Cross-entropy">cross-entropy</Link> loss that describes the difference between the predictions and the actual distribution provided by the human labels is given by:
            <br/><br/>
            <MathJaxContext>
                <MathJax>
                    {
            ...
            <br/>
            <MathJaxContext>       
            where  <MathJax inline>{"\\(\\mu_1\\)"}</MathJax> and <MathJax inline>{"\\(\\mu_2\\)"}</MathJax> are the actual probability distribution over the two sequences, indicating which segment was preferred by the human.  This can be [1, 0] if the first segment is preferred, [0, 1] if the second segment is preferred, or [0.5, 0.5] if there is no preference. 
            </MathJaxContext>     
            ...
            <br/><br/>
            Another nice bonus from working with such a simple problem is that we don't even need to use the gym environment to generate the preferences database.  In total there are five actions that we can take, the null action (do nothing), or moving up, down, left or right.  The code for this subsection is in <Code p="0">make_preferences_db.py</Code>.  We create a simple dictionary that maps the index of each action with the movement direction in cartesian coordinates of the pixel.
            ...
            Each row of the database therefore contains an index, the two segments, and the distribution over the preference.  When training the reward model we will sample from this database.  The code for handling the database is in <Code p="0">db.py</Code>, which we won't go into here but is just using standard SQL queries to store and retrieve the entries.


            <h3 id="reward" className="mt-5" style={{fontSize: "20px"}}>Learning the Reward Model</h3><br />

            When training the reward model we will be using a simple fully-connected network with a single hidden layer with Tanh activation functions.  This was chosen to match the architecture used in the original PPO implementation (see <Link style={{textDecoration: "underline"}} href="https://iclr-blog-track.github.io/2022/03/25/ppo-implementation-details/">here</Link> for more details), and will be the same as we use in the A2C network for the actual training.  In this case however there is only a single output value representing the reward, and the input is an observation-action pair.
            ...
            <MathJaxContext>
            Below we illustrate the progress of the training by testing how well the reward model is able to reflect the preferences.  At regular intervals we take the entire preference table, and split the entries into three groups, where segment one is preferred, segment two is preferred and where there is no preference.  We calculate the reward for each segment and then histogram <MathJax inline>{"\\(R_1 - R_2\\)"}</MathJax>.  If the reward model is working, then <MathJax inline>{"\\(R_1\\)"}</MathJax> should be greater than <MathJax inline>{"\\(R_2\\)"}</MathJax> when segment one is preferred, and so those points should all have positive values.  Similarly when segment two is preferred those points should all have negative values, and when there is no preference those points should be clustered around zero.
            </MathJaxContext>
            <br/><br/>
            ...
            To train our agent to play the moving dot game we start with an implementation of an A2C network using PPO, which you can find <Link style={{textDecoration: "underline"}} href="https://github.com/vwxyzjn/ppo-implementation-details/blob/main/ppo.py">here</Link>.  It uses a seperate network for the agent and the critic, each of which has the same design as in our reward model. We won't go into the details of this file in this post, but will simply describe how to use the reward network trained previously in place of the standard reward provided by the gym environment.

            <br/><br/> 

            Firstly we add an extra argument that the user can pass <Code p="0">--rlhf</Code> which will initialize the <Code p="0">reward_network</Code> from the file saved in the previous step.  It then sets the network to eval mode as we won't be doing any updating of the weights from this point.
            ...
            We run this process for 1000000 timesteps, with and without the additional RLHF parameter for 5 random initialisations each.
