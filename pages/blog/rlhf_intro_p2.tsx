...
`
    return (
        ...
        <div className="container">
            ...
            <h1 className="h1 text-center mb-0 pt-3 font-weight-bold text-body">Intro to Reinforcement Learning From Human Feedback (part 2)</h1>
            ...
            In our previous post we introduced some of the basic features of RLHF using the moving-dot gym environment.  In that case we could create a reward model by generating preferences between individual state-action pairs, and preferring the state that moved the dot closer to the center. For most interesting cases however, individual state-action pairs will not include enough information for a human to be able judge which is better. In their <Link style={{textDecoration: "underline"}}   href = "https://arxiv.org/pdf/1706.03741.pdf">paper</Link> (henceforth C17) the authors use sequences of state-action pairs (referred to as segment trajectories) and elicit preferences on those, balancing longer sequences which have more context, with the training time associated with having to learn from those longer sequences. 
            <br/><br/>
            In this post then we will be using the <Link style={{textDecoration: "underline"}}   href = "https://gymnasium.farama.org/environments/classic_control/mountain_car/">Mountain Car</Link> gym environment and using segment trajectories that each have 30 state-action pairs, corresponding to about 1 second of video which is similar in length to the trajectories in C17.  In addition to using multiple states, we will also introduce a couple of extra features used in C17:
            <br/><br/>
            <ul>
                <li>
                    An ensemble of three reward models -  Each is trained separately and we average the rewards from the three networks to produce the final reward.
                </li>
                <li>
                    A 10% chance of replacing the probability distribution of the preferences for a particular pair of trajectory segments with 50/50.  
                </li>
            </ul>

            The code that we will describe can be found in our GitHub repo for this post <a style={{textDecoration: "underline"}} href="https://github.com/daoplays/RLHF/tree/main/mountain_car">here</a>. 
            ...
            <ul>
                ...
                <li>
                    <HStack><Text align="center" m="0" p="0">Training our mountain car agent given the reward model </Text><a href="#training"><AiOutlineArrowRight/></a></HStack>
                </li>
            </ul>
            ...
            We keep the same basic network architecture as in the previous post when training the mountain car reward model: A simple fully-connected network with a single hidden layer with Tanh activation functions.  In the previous post we clipped the rewards post hoc to the range -1 to 1 to match the paper, however in this case we just added a Tanh function to the output to normalise the rewards for us.



            <HighLightCode codeBlock={reward_network_1} language={"python"}/>     

            The loss function is then mathematically identical to the previous post, however just needs to be generalised to accept a sequence of state-action pairs, rather than single ones.  In addition, rather than taking the exponential of the sum of rewards directly, we make a simple re-formulation that leaves the answer unchanged but is numerically more stable as we are only taking an exponential of the difference between the sum and the mean.
            ...
            million steps.  In addition we include one extra feature from C17, that 10% of trajectory pairs have their probability distribution replaced with an even preference at random:
