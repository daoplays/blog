    ...
    return (
        <div className="home" style={{ fontSize: 18 }}>
            <div className="container">
                ...
                <ul>
                <li>
                    <HStack><Text align="center" m="0" p="0">Setting up the extension in the mint</Text> <a href="#init_mint"><AiOutlineArrowRight/></a></HStack>
                </li>
                <li>
                    <HStack><Text align="center" m="0" p="0">Transferring tokens that make use of the extension</Text><a href="#transfer_tokens"><AiOutlineArrowRight/></a></HStack>
                </li>
                <li>
                    <HStack><Text align="center" m="0" p="0">Writing the actual transfer hook program following the required
                interface </Text><a  href="#hook_program"><AiOutlineArrowRight/></a></HStack>
                </li>
            </ul>
                ...
                A discriminator is a set of eight bytes that can be defined by the user in any way they see fit. The crate provides some useful macros that allow us to construct them directly from strings, so for
                ...
                <br />
                Using this macro provides the struct with the <Code p="0">SPL_DISCRIMINATOR_SLICE</Code> property, which we can use to distinguish between instructions.  For example, when our transfer hook program is used, we must unpack the input that has been sent to the program to check what instruction has been called, and can use this property to do this:
                ...
                In the above example you can see that we are making use of the 'Pod' attribute for struct <Code p="0">MyPodValue</Code>.  The POD data type is described in detail in the bytemuck crate{" "}
                <a style={{ textDecoration: "underline" }} href="https://docs.rs/bytemuck/latest/bytemuck/">
                    here.
                </a>{" "}
                Going into the weeds too much on this point is a bit out of scope for this post, but it is usefull to at least mention it's existence as you will see the code for the transfer hook makes use of things like{" "}
                <Code p="0">
                    {" "}
                    <a
                        style={{ textDecoration: "underline" }}
                ...
                structure has no padding bytes, or that you can make a copy simply by copying the bytes. The default Bool type in Rust, and the AccountMeta type defined in the Solana crate do not satisfy these requirements, and so special versions have been implemented that do.

                <h3 id="tokens-header" className="mt-5" style={{ fontSize: "20px" }}>
                    Type-Length-Value (TLV)
                </h3>

                TLV structures are defined in the <Link style={{ textDecoration: "underline" }} href="https://docs.rs/spl-type-length-value/0.3.0/spl_type_length_value/">this</Link> crate, and are used in a <Link  style={{ textDecoration: "underline" }} href="https://en.wikipedia.org/wiki/Type%E2%80%93length%E2%80%93value">wide range</Link> of applications.  They provide a way of specifying the makeup of a slab of bytes, by specifying the type (8 bytes, using a discriminator), the length of the data (4 bytes), and then following that with the data itself as a slab of bytes of the given length. This way you can easily fill up an array of different data structures, each in the TLV format, and iterate through the structures to deserialise the data.  The type then provides functionality to get the value (i.e to automatically deserialise the bytes to the specified format), or to get the discriminators to check the type.  We will give an example of this below for the <Code p="0">ExtraAccountMeta</Code> structure.
                ...
                Setting up the transfer hook extension in the token mint starts in the same fashion as setting up any of the other extensions.  You can find the full code for this section in the git repo for the previous blog post <Link style={{ textDecoration: "underline" }} href="https://github.com/daoplays/solana_examples/tree/master/token_2022/program">here</Link> here. We first simply have to invoke the <Link style={{ textDecoration: "underline" }} href="https://docs.rs/spl-token-2022/latest/spl_token_2022/extension/transfer_hook/instruction/fn.initialize.html">initialize</Link> method for the transfer hook extension:
                ...
                Here <Code p="0">transfer_hook_program_account</Code> is the address of the deployed transfer hook program that will be called whenever a transfer occurs, which we will describe in the final section below.  The extra step that is required to set up the transfer hook extension is to then call the function that will set up the meta data that describes the additional accounts that should be passed in order to use the transfer hook.  We follow the example interface and call this <Code p="0">InitializeExtraAccountMetas</Code>.

                <br/><br/>
                <HighLightCode codeBlock={init_account_metas} language={"rust"}/>

                This follows a pretty standard setup for a CPI call. We have an extra optional account, <Code p="0">mint_data_account_info</Code>, which we will use in this post to count the number of transfers for each mint created by the program.  This is a unique account per mint, and uses the mint address and the string "mint_data" as the seeds (this will be shown explicitly in the transfer hook program section).

                <h2 id="transfer_tokens" className="mt-5" style={{ fontSize: "25px" }}>
                    Transferring Tokens
                </h2>

                Transferring a token that has the transfer hook extension isn't quite as trivial as for the other extensions.  The Solana blockchain requires that all accounts involved in a transaction are included as part of that transaction, but the default <Code p="0">transfer_checked</Code> function doesn't know anything about our transfer hook program, or the data account that we will be modifying when that program is called.  We will therefore to modify the instruction before we can invoke it.

                <br/><br/>

                We start by creating an instance of the <Code p="0">transfer_checked</Code> instruction as normal, except that we have made it mutable.  This means we can modify the set of accounts the instruction knows about afterwards:
                ...
                Once these have been pushed onto the accounts vector for the instruction we can just call invoke and the transfer will happen.  Note that the <Code p="0">spl_tlv_account_resolution</Code> crate does include some helper functions to do this more automatically (e.g. <Code p="0">add_to_cpi_instruction</Code>) however in this case we found it simpler, and more instructive to just do it ourselves.

                <h2 id="hook_program" className="mt-5" style={{ fontSize: "25px" }}>
                    The Transfer Hook Program
                </h2>

                In this final section we will implement the transfer hook program itself, the code for this section can be found in our git repo <Link style={{ textDecoration: "underline" }} href="https://github.com/daoplays/solana_examples/tree/master/transfer_hook">here</Link>.  In our case we will be building two functions, both of which we have mentioned previously:
                <br/><br/>
                <ul>
                    <li>
                ...
                Once the account has been created we then define the <Code p="0">ExtraAccountMeta</Code> structure that will store information about that account on chain. The <Code p="0">ExtraAccountMeta</Code> structure has several different methods for instantiation depending on the type of account that is being saved.  In this case we have made a PDA that is owned by our transfer hook program, and so we use the <Code p="0"><Link style={{ textDecoration: "underline" }}  href="https://docs.rs/spl-tlv-account-resolution/latest/spl_tlv_account_resolution/account/struct.ExtraAccountMeta.html#method.new_with_seeds">new_with_seeds</Link></Code> function, which makes use of the <Code p="0"><Link style={{ textDecoration:"underline" }}  href="https://docs.rs/spl-tlv-account-resolution/latest/spl_tlv_account_resolution/seeds/enum.Seed.html">Seed</Link></Code> enum to save how to construct the address of the account.  In our case we have a 
                <Code p="0">Literal</Code> type seed that comes from the text "mint_data", and a seed that comes from an <Code p="0">AccountKey</Code> (the mint account) which is defined using the index of that account as it was passed to this function (in our case that was account 1).  Using those seeds we can instantiate the <Code p="0">ExtraAccountMeta</Code> data and increment <Code p="0">n_extra_accounts</Code>:
                <br/><br/>
                <HighLightCode codeBlock={init_accounts_2_1} language={"rust"}/>
                <br />
                <br />
                That is most of the hard work done.  Now we know how many extra accounts we have, we can determine the size of the account that will hold that data using the <Code p="0">size_of</Code> function provided by <Code p="0"><Link style={{ textDecoration: "underline" }} href="https://docs.rs/spl-tlv-account-resolution/latest/spl_tlv_account_resolution/state/struct.ExtraAccountMetaList.html">ExtraAccountMetaList</Link></Code> structure, and then create the account as normal:
                ...
                The final function to implement then is the Execute function, which is the thing that will actually be called every time a transfer occurs.  In our case because this either does nothing (if no mint_data was passed when the mint was being created), or simply incrments a counter in a data account.  As such it is really quite simple.  The body of the function is defined in the <Code p="0">process_execute</Code> function.  Note that this gets passed the amount that has been transferred, however we don't use it at all in this example.

                <br/><br/>
                <HighLightCode codeBlock={execute_1} language={"rust"}/>
                <br /><br />
                
                To determine whether we have a mint_data account we check the account that we created in the previous step that contains the <Code p="0">ExtraAccountMetaList</Code>.  This involves creating a type-length-value object from the data, and then unpacking the bytes as the <Code p="0">ExtraAccountMetaList</Code> object.  In our case we only care if there are any accounts so we just check the length.  If it is greater than zero we know that there must have been a mint data account passed, and so we enter that code block.
                ...
                <Tokens2022 />
                <br />
                Hopefully you have learnt something new about using the transfer hook with the Token-2022 program in this post. We will aim to follow this up at some point with some more interesting examples, such as evolving pokemon type characters in a game. If you don't to miss that then go ahead and follow us on{" "}
