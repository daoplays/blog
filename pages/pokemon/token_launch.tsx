...
function PostContent() {
    ...
    return (
        ...
        <div className="container">
            ...
            For a more detailed description of how the token launch program works please see  <Link style={{textDecoration: "underline"}} href="/blog/charity_token_launch">this</Link> blog post.  The only difference between the version described in that post and the version here is that the Play Tokens have had their mint authority retained.  This is simply to ensure that in the unlikely event that users simply hold onto their tokens, rather than returning them to the pool by using them to vote for moves, more can be released later for legitimate players.

            <br/><br/>

            This token launch has been funded with 50 million Play Tokens, which is enough for between 25 and 50 thousand participants, and will shut down when those tokens have been exhausted.  From that point on the only way to receive Play Tokens from DaoPlays directly will be by taking part in the token auction with other users from the <Link style={{textDecoration: "underline"}} href="/pokemon/pokemon">main app page</Link> (for more details please see our blog post on the DaoPlays Pokemon app <Link style={{textDecoration: "underline"}} href="/blog/pokemon_guide">here</Link>).
